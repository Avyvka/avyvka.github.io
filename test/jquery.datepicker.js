    ...
    var DatePicker = function ($el, options) {
        ...
        var core = {
            ...
            _bindEvent: function () {
                ...
                if (datepicker.$timePanel && datepicker.$timePanel.length > 0) {
                    datepicker.$timePanel.on('click.datepicker', '.gmi-time-panel__body__item--spinner__item:not(.disabled)', function (e) {
                        ...
                        const scrollTop = offsetTop + (elementHeight / 2) - (wrapperHeight / 2);

                        $timeItem.addClass('active').siblings().removeClass('active');
                        $timeItemWrapper.scrollTop(scrollTop);
                        e.stopPropagation();

                        datepicker.$timePanel.find(`.gmi-time-panel__btn[data-role="confirm"]`).trigger('click.datepicker');
                    });
                    ...
                    datepicker.$timePanel.on('click.datepicker', '.gmi-time-panel__btn', function (e) {
                        ...
                        switch (role) {
                            case 'confirm':
                                ...
                                datepicker.$pickerPanel.find(`.gmi-picker-panel__link-btn[data-role=determine]`).trigger('click.datepicker');
                                break;
                            case 'cancel':
                                $delegateTarget.hide();
                                break;
                ...
                datepicker.$pickerPanel.on('click.datepicker', '.gmi-date-picker__header__icon-btn', function (e) {
                    ...
                    switch (action) {
                        ...
                        case 'next':
                            ...
                            break;
                        case 'change':
                            core._setChangeButtonAction($delegateTarget, currentView);
                            break;
                        default:
                            break;
                    }
                ...
                datepicker.$pickerPanel.on('click.datepicker', '.gmi-picker-panel__link-btn:not(.gmi-time-panel__btn)', function (e) {
                    ...
                    switch (role) {
                        case 'determine':
                            ...
                            core._setDate(value);
                            if (e.originalEvent) core._hidePickerPanel();
                            break;
                        case 'now':
            ...
            _generateDateDOM: function () {
                ...
                var dateDomStr = '<div data-role="' + type + '" class="gmi-picker-panel gmi-date-picker custom ' + hasTimeClass + '" style="display: none;">' +
                    '<div class="gmi-picker-panel__body">'
                    + datetimeHeaderStr +
                    '<div class="gmi-picker-panel__body__left">' +
                    '<div class="gmi-picker-panel__body__header">' +
                    '<span class="gmi-date-picker__header__label--year">' + yearLabel + '</span>'
                    + monthDom +
                    '<em data-action="change" class="gmi-date-picker__header__icon-btn gmi-picker-panel__btn--datetime"></em>' +
            ...
            _generateDatetimeHeader: function () {
                var datetimeHeaderStr = '<div class="gmi-picker-panel__body__right"><ul class="gmi-picker-panel__body__header--time">' +
            ...
            _getDateTable: function (nowDate) {
                ...
                for (var i = 0; i < rows.length; i++) {
                    ...
                    for (var j = 0; j < row.length; j++) {
                        ...
                        var text = (cell.type === 'today' && datepicker.todaySuffix) ? datepicker.todaySuffix : cell.text;
            ...
            _setChangeButtonAction: function ($delegateTarget, currentView) {
                $delegateTarget.toggleClass("view-time");
                const $panelBody = $delegateTarget.find(".gmi-picker-panel__body");
                if ($delegateTarget.hasClass("view-time")) {
                    $panelBody.scrollLeft($panelBody.width());
                } else {
                    $panelBody.scrollLeft(-$panelBody.width());
                }
            },
            ...
            _setNewDateDOM: function ($delegateTarget, year, month, date) {
                ...
                const $newDateTable = $(newDateTableStr);
                // $newDateTable.appendTo($tableWrapper).find('td').removeClass('current').filter(function () {
                $newDateTable.appendTo($tableWrapper).show().find('td').removeClass('current').filter(function () {
            ...
            _getDatePanelPosition: function () {
                ...
                var panelWidth = datepicker.$pickerPanel.outerWidth();
                ...
                if ((top - scrollTop > panelHeight) && (top - scrollTop + elHeight + panelHeight > viewHeight)) {
                    ...
                    transformOrigin.y = "top";
                }
                
                if ((left - scrollLeft > panelWidth) && (left - scrollLeft + elWidth + panelWidth > viewWidth)) {
                    left -= panelWidth + Number(datepicker.$pickerPanel.css('margin-left').replace(/px/, '')) * 2;
                    left += elWidth;
                ...
                datepicker.$pickerPanel.css("transform-origin", `${transformOrigin.x} ${transformOrigin.y}`)
                ...
                if (datepicker.$pickerPanel.length > 0) {
                    ...
                    switch (datepicker.align) {
                        ...
                        case 'right':
                            ...
                            break;
                        case "custom":
                            left = (position.left + panelWidth + edgePadding > document.body.clientWidth) ? Math.abs(document.body.clientWidth - panelWidth) / 2 : position.left;
                            break;
                        default:
                            break;
                    }
            ...
            _showPickerPanel: function () {
                ...
                datepicker.$pickerPanel.removeClass("view-time");
                datepicker.$pickerPanel.find(".gmi-picker-panel__body").scrollLeft(-10000);
            ...
            _elClickHandler: function (e) {
                ...
                if (datepicker.$pickerPanel.is(':hidden')) {
                    core._showPickerPanel();
                } else {
                    core._hidePickerPanel();
                }
